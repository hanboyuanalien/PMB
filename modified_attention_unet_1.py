...
class modified_att_unet_1(nn.Module):
    def __init__(self,img_ch=1,output_ch=1):
        super(modified_att_unet_1,self).__init__()
        ...
        self.Att5 = Attention_block(F_g=1,F_l=512,F_int=256) # F_g comes from CAM and has only one channel
        self.Up_conv5 = conv_block(ch_in=1024, ch_out=512)

        self.Up4 = up_conv(ch_in=512,ch_out=256)
        self.Att4 = Attention_block(F_g=1,F_l=256,F_int=128) # F_g comes from CAM and has only one channel
        self.Up_conv4 = conv_block(ch_in=512, ch_out=256)
        
        self.Up3 = up_conv(ch_in=256,ch_out=128)
        self.Att3 = Attention_block(F_g=1,F_l=128,F_int=64) # F_g comes from CAM and has only one channel
    ...
    def forward(self,x,cam1,cam2,cam3,cam4): 
        # encoding path
        ...
        x4_att = self.Att5(cam1,x4) 
        d5 = torch.cat((x4_att,d5),dim=1) 
        d5 = self.Up_conv5(d5)
        
        d4 = self.Up4(d5) # d4 torch.Size([2, 256, 96, 96])
        x3_att = self.Att4(cam2,x3)
        d4 = torch.cat((x3_att,d4),dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4) # d3 torch.Size([2, 128, 192, 192])
        x2_att = self.Att3(cam3,x2)
        d3 = torch.cat((x2_att,d3),dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3) # d2 torch.Size([2, 64, 384, 384])
        x1_att = self.Att2(cam4,x1)
